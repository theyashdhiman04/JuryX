...
const UserUpload = () => {
  ...
  const processFiles = async (fileList: File[]) => {
    ...
    try {
      ...
      const formData = new FormData();
      formData.append("project", zipBlob, "project.zip");
      formData.append("userId", String(user?.email)); // Ensure this matches your backend expectation (ID vs Email)

      // Upload
      await axios.post("/api/users/userFileUpload", formData, {
          ...
          <div className="p-8">
                ...
                <AnimatePresence mode="wait">
                  {loading ? (
                    ...
                    <motion.div
                      ...
                      animate={{ opacity: 1 }}
                      exit={{ opacity: 0 }}
                      className="flex flex-col items-center text-center px-4 pointer-events-none"
                    >
                      <UploadCloud
                        className={`w-10 h-10 mb-3 transition-colors ${
                          dragActive
            ...
            <div className="mt-6 flex items-start gap-3 p-3 rounded-lg bg-zinc-950/50 border border-zinc-800/50">
              ...
              <div className="text-xs text-zinc-400 leading-relaxed">
                ...
                or configuration files. Node_modules will be ignored
                automatically by the compressor.
              </div>
            </div>
          </div>
        </div>
